...
public class ManagedSnapshotCacheTests
{
    ...
    public async Task CachedManagedHitSkipsTheOwnershipProbe()
    {
        ...
        bool first = cache.TryGetManaged(owner,
            _ => { ownershipProbes++; return true; },
            _ => ++builds,
            out int firstValue);
        bool repeated = cache.TryGetManaged(owner,
            _ => { ownershipProbes++; return false; },
            _ => ++builds,
    ...
    public async Task UnmanagedMissFallsThroughWithoutPublishingASnapshot()
    {
        ...
        bool found = cache.TryGetManaged(owner, _ => false,
            _ => ++builds, out int value);
    ...
    public async Task RemovalRequiresOwnershipToBeProvenAgain()
    {
        ...
        int ownershipProbes = 0;
        var cache = new ManagedSnapshotCache<Owner, int>();
        cache.TryGetManaged(owner,
            _ => { ownershipProbes++; return true; }, _ => 7, out _);

        cache.Remove(owner);
        bool found = cache.TryGetManaged(owner,
            _ => { ownershipProbes++; return false; }, _ => 9, out _);
