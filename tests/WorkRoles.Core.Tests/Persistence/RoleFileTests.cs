...
public class RoleFileTests
{
    ...
    public async Task NowhereLocationRoundTripsWithoutBecomingAnywhere()
    {
        var document = new RoleFileDocument
        {
            roles =
            {
                new FileRole { label = "Disabled by migration", locations = { LocationRules.Nowhere } },
            },
        };
    ...
    [Test]
    public async Task CompositeRoleRoundTripsMembersInOrder()
    {
        var document = new RoleFileDocument
        {
            roles =
            {
                new FileRole
                {
                    label = "Core+",
                    composite = true,
                    members = { new FileRoleReference("role-2", "Pyrophobe"), new FileRoleReference("role-3", "Core") },
                },
            },
        };
        ...
        await Assert.That(string.Join("|", role.members.Select(member => member.fileId + ":" + member.label))).IsEqualTo("role-2:Pyrophobe|role-3:Core");
        await Assert.That(role.entries.Count).IsEqualTo(0);
    }

    [Test]
    public async Task CompositeRoleDropsHandEditedJobsOnParse()
    {
    ...
    {
        RoleFileDocument parsed = RoleFile.Parse("<WorkRoles version=\"7\"><Roles>" + "<Role fileId=\"role-a\" id=\"template-a\" name=\"   \"><Jobs/></Role>" + "</Roles></WorkRoles>");
    ...
    public async Task LegacyPublicCollectionFieldTypesRemainExact()
    {
        ...
        await Assert.That(document.GetField(nameof(RoleFileDocument.groups))!.FieldType).IsEqualTo(typeof(List<string>));
        await Assert.That(document.GetField(nameof(RoleFileDocument.recommendationOrder))!.FieldType).IsEqualTo(typeof(List<string>));
        await Assert.That(path.GetField(nameof(FileTrainingPath.entries))!.FieldType).IsEqualTo(typeof(List<(string role, int min, int max)>));
    }
    ...
    public async Task StaleRichMetadataFallsBackToMutatedLegacyCollections()
    {
        var doc = new RoleFileDocument
        {
            ...
            groupsWithIds =
            {
                new FileGroup { fileId = "group-old", name = "Old team" },
            },
            roles =
            {
                new FileRole { fileId = "role-new", label = "Renamed role" },
            },
            trainingPaths =
            {
                new FileTrainingPath
                {
                    ...
                    entriesWithIds = { new FileTrainingPathEntry("role-old", "Old role", 0, 21) },
                },
            },
            recommendationOrder = { "Renamed role" },
            recommendationOrderWithIds = { new FileRoleReference("role-old", "Old role") },
        };
    ...
    public async Task ParserPopulatesAlignedLegacyAndRichCollections()
    {
        ...
        await Assert.That(parsed.groups).IsEquivalentTo(["Team"]);
        ...
        await Assert.That(parsed.trainingPaths[0].entriesWithIds[0].role.fileId).IsEqualTo("role-a");
        await Assert.That(parsed.trainingPaths[0].anchorWithId.fileId).IsEqualTo("role-a");
        await Assert.That(parsed.recommendationOrder).IsEquivalentTo(["Worker"]);
        await Assert.That(parsed.recommendationOrderWithIds[0].fileId).IsEqualTo("role-a");
    }

    private static FileRole Full() =>
        new()
        {
            label = "Night Guard",
            ...
            entries = [new(JobEntryKind.WorkGiver, "FightFires"), new(JobEntryKind.WorkType, "Hauling")],
        };
    ...
    public async Task FullDocumentRoundTrips()
    {
        var doc = new RoleFileDocument
        {
            ...
            {
                Full(),
                new FileRole { label = "Plain", entries = { new(JobEntryKind.WorkType, "Mining") } },
            },
        };
        ...
        await Assert.That(parsed.palette[0].color.Hex()).IsEqualTo("#99541a");
        ...
        await Assert.That(string.Join("|", role.locations)).IsEqualTo("caravans|settlement:Bö & <Wood> \"Camp\"|ship:The Wanderer");
        await Assert.That(string.Join(",", role.entries.Select(e => e.Encode()))).IsEqualTo("WorkGiver:FightFires,WorkType:Hauling"); // ORDER preserved across kinds
    ...
    [Test]
    public async Task DefaultsProduceMinimalOptionsAndScaffoldingIsAlwaysPresent()
    {
        var doc = new RoleFileDocument
        {
            roles =
    ...
    [Test]
    public async Task GroupsRoundTripInOrderWithSpecialCharactersAndDefaultStaysImplicit()
    {
        var doc = new RoleFileDocument
        {
            ...
            roles =
            {
                new FileRole
                {
                    label = "A",
                    group = "Älpha",
                    entries = { new(JobEntryKind.WorkType, "Mining") },
                },
                ...
                new FileRole
                {
                    label = "C",
                    group = "Unlisted",
                    entries = { new(JobEntryKind.WorkType, "Cooking") },
                },
            },
        };
        var parsed = RoleFile.Parse(RoleFile.Build(doc));
        ...
        await Assert.That(string.Join("|", parsed.groups)).IsEqualTo("Zulu & \"Friends\"|Älpha");
    ...
    {
        var path = new FileTrainingPath { name = "Frontline", entries = { ("Shooter", 0, 8), ("Deleted", 5, 10), ("Sniper", 6, 21), ("Shooter", 2, 4) } };
        ...
        var resolved = RoleFile.ResolvePathEntries(
            path,
            name =>
                name == "Shooter" ? 1
                : name == "Sniper" ? 2
    ...
    public async Task TrainingPathsAndRecommendationOrderRoundTrip()
    {
        var doc = new RoleFileDocument
        {
            ...
            roles =
            {
                new FileRole
                {
                    label = "Shooter",
                    group = "Combat",
                    entries = { new(JobEntryKind.WorkType, "Hunting") },
                },
                new FileRole
                {
                    label = "Sniper",
        ...
        await Assert.That(string.Join("|", first.entries.Select(e => $"{e.role}:{e.min}-{e.max}"))).IsEqualTo("Shooter:0-8|Sniper:6-21");
        ...
        await Assert.That(string.Join("|", second.entries.Select(e => $"{e.role}:{e.min}-{e.max}"))).IsEqualTo("Doctor:4-21");
        await Assert.That(string.Join(",", parsed.recommendationOrder)).IsEqualTo("Doctor,Shooter,Sniper");
    }
    ...
    public async Task AnchorlessPathAndEmptyOrderStayAbsent()
    {
        var doc = new RoleFileDocument
        {
            roles =
            {
                new FileRole { label = "A", entries = { new(JobEntryKind.WorkType, "Mining") } },
            },
            trainingPaths =
            {
                new FileTrainingPath { name = "Solo", entries = { ("A", 0, 21) } },
            },
        };
        string xml = RoleFile.Build(doc);
    ...
    public async Task FilesWithoutVersionOrNewSectionsParseAsBefore()
    {
        ...
        var parsed = RoleFile.Parse(
            "<WorkRoles><Palette/><Roles>" + "<Role name=\"Ok\"><Options><Color>red-800</Color></Options>" + "<Jobs><WorkType>Mining</WorkType></Jobs></Role></Roles></WorkRoles>"
        );
        await Assert.That(parsed.error == null).IsTrue();
        await Assert.That(parsed.roles.Count).IsEqualTo(1);
    ...
    public async Task PathParsingSkipsNamelessPathsAndBadBands()
    {
        var parsed = RoleFile.Parse(
            "<WorkRoles version=\"3\"><Palette/>"
                ...
                + "<Path name=\"Ok\"><Role min=\"0\" max=\"3\">A</Role>"
                + // span < 4
        ...
        await Assert.That(string.Join("|", parsed.trainingPaths[0].entries.Select(e => e.role + ":" + e.min + "-" + e.max))).IsEqualTo("B:4-12");
    }
    ...
    public async Task DuplicateGroupNamesDedupCaseInsensitively()
    {
        ...
        await Assert.That(parsed.error == null).IsTrue();
        await Assert.That(parsed.groups).IsEquivalentTo(["Kitchen", "Farm"]);
    }
    ...
    [Test]
    public async Task GarbageDocumentReportsAnError()
    {
        await Assert.That(RoleFile.Parse("not xml at all").error != null).IsTrue();
    }

    [Test]
    public async Task EmptyDocumentReportsNoValidRoles()
    {
        await Assert.That(RoleFile.Parse("<WorkRoles version=\"1\"/>").error).IsEqualTo("document contains no valid roles");
    }
    ...
    [Test]
    public async Task HexCodecRoundTripsValidColors()
    {
    ...
    [Arguments("#99551")]
    [Arguments("#zzzzzz")]
    public async Task HexCodecRejectsInvalidColors(string input)
    {
        await Assert.That(ColorRgb.TryParseHex(input, out _)).IsFalse();
    }

    [Test]
    public async Task StableIdsDisambiguateDuplicateLabelsAcrossReferences()
    {
        var first = new FileRole
        {
            fileId = "role-a",
            label = "Worker",
            groupId = "group-a",
        ...
        var doc = new RoleFileDocument
        {
            ...
            trainingPaths =
            {
                new FileTrainingPath
                {
                    ...
                    entries = { ("Worker", 0, 8), ("Worker", 8, 21) },
                    entriesWithIds = { new FileTrainingPathEntry("role-a", "Worker", 0, 8), new FileTrainingPathEntry("role-b", "Worker", 8, 21) },
                },
            },
            recommendationOrder = { "Worker", "Worker" },
            recommendationOrderWithIds = { new FileRoleReference("role-b", "Worker"), new FileRoleReference("role-a", "Worker") },
        };
        ...
        var resolved = RoleFile.ResolvePathEntries(
            ...
            parsed,
            role =>
                role.fileId == "role-a" ? 101
                : role.fileId == "role-b" ? 102
                : null
        );
        int[] resolvedOrder = parsed
            .recommendationOrderWithIds.Select(reference =>
            {
    ...
    public async Task MalformedAndMissingIdsFallBackToLabels()
    {
        ...
        await Assert.That(RoleFile.ResolveRole(parsed, "missing-role", "Beta")?.label).IsEqualTo("Beta");
        await Assert.That(string.Join(",", resolved.ids)).IsEqualTo("2,1");
    }

    [Test]
    public async Task UniqueRoleIdRemainsAuthoritativeWhenDisplayLabelIsStale()
    {
        RoleFileDocument parsed = RoleFile.Parse(
        ...
        await Assert.That(RoleFile.ResolveRole(parsed, "role-a", "Beta")?.fileId).IsEqualTo("role-a");
    }

    [Test]
    public async Task UniqueGroupIdRemainsAuthoritativeWhenDisplayLabelIsStale()
    {
        RoleFileDocument parsed = RoleFile.Parse(
            "<WorkRoles version=\"7\"><Groups>"
                + "<Group fileId=\"group-a\" name=\"First\"/><Group fileId=\"group-b\" name=\"Second\"/>"
        ...
        await Assert.That(RoleFile.ResolveGroup(parsed, "group-a", "Second")?.fileId).IsEqualTo("group-a");
    }
    ...
    public async Task VersionSixLabelOnlyReferencesRemainCompatible()
    {
        ...
        var resolved = RoleFile.ResolvePathEntries(parsed.trainingPaths[0], parsed, role => runtimeIds.TryGetValue(role, out int id) ? id : null);
        var orderRole = RoleFile.ResolveRole(parsed, parsed.recommendationOrderWithIds[0].fileId, parsed.recommendationOrder[0]);
        ...
        await Assert.That(resolved.ids).IsEquivalentTo([73]);
    ...
    public async Task PublicResolversTolerateNullCollectionsAndElements()
    {
        ...
        await Assert.That(RoleFile.ResolveRole(document, "role-a", "Cook")).IsSameReferenceAs(role);
        await Assert.That(RoleFile.ResolveRole(document, "missing", "cook")).IsSameReferenceAs(role);
        await Assert.That(RoleFile.ResolveGroup(document, "group-a", "Crew")).IsSameReferenceAs(group);
        await Assert.That(RoleFile.ResolveGroup(document, "missing", "crew")).IsSameReferenceAs(group);
