...
public class OwnerGenerationRegistryTests
{
    ...
    {
        var registry = Registry();
        registry.Begin("oldest");
        registry.Touch("tip", "same text", 1);
        registry.End("oldest");
        registry.Begin("middle");
        registry.Touch("tip", "same text", 2);
        registry.End("middle");
        registry.Begin("head");
        registry.Touch("tip", "same text", 3);
        registry.End("head");
    ...
    public async Task ManyAffectedLookupKeysRebuildOnlyTheirCollisionBuckets()
    {
        ...
        await Assert.That(CountingLookupKey.Comparisons).IsLessThan(keyCount * 8).Because("collision restoration must not compare every remaining entry for every lookup key");
    ...
    public async Task InvalidGenerationNestingFailsDeterministically()
    {
        ...
        await Assert.That(() => registry.Begin("preview")).Throws<InvalidOperationException>();
        await Assert.That(() => registry.End("preview")).Throws<InvalidOperationException>();

        registry.Touch("tip", "text", 1);
        registry.End("main");

        await Assert.That(() => registry.Touch("tip", "text", 2)).Throws<InvalidOperationException>();
        await Assert.That(() => registry.End("main")).Throws<InvalidOperationException>();
    }

    private static OwnerGenerationRegistry<string, string, string, int> Registry() => new OwnerGenerationRegistry<string, string, string, int>();
