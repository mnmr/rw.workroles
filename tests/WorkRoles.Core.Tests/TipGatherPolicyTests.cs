...
public class TipGatherPolicyTests
{
    ...
    {
        await Assert.That(TipGatherPolicy.ShouldGather(
            refresh, hasText: false, frame: 100, lastObservedFrame: 100)).IsTrue();
    }
    ...
    {
        await Assert.That(TipGatherPolicy.ShouldGather(
            TipRefresh.Pinned, hasText: true, frame: 5000, lastObservedFrame: 10)).IsFalse();
    }
    ...
    {
        await Assert.That(TipGatherPolicy.ShouldGather(
            TipRefresh.PerSession, hasText: true,
            frame: 100 + frameGap, lastObservedFrame: 100)).IsFalse();
    }
    ...
    public async Task SessionRegathersAfterADisplayGap()
    {
        await Assert.That(TipGatherPolicy.ShouldGather(
            TipRefresh.PerSession, hasText: true,
            frame: 102, lastObservedFrame: 100)).IsTrue();
    }
}
