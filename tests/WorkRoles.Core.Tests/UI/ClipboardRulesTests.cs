...
public class ClipboardRulesTests
{
    ...
    public async Task ForeignOwnerClipboardDataIsEmpty()
    {
        ...
        Assignment[] stored = [new(7, enabled: true, pinned: false)];

        var snapshot = ClipboardRules.SnapshotForOwner(owner, foreignOwner, stored, Clone);
    ...
    public async Task MissingNullAndDuplicateRoleIdsAreFilteredDeterministically()
    {
        ...
        var filtered = ClipboardRules.FilterValidDistinct(source, assignment => assignment?.RoleId, [1, 2, 5], Clone);

        await Assert.That(string.Join(",", filtered.Select(assignment => assignment.RoleId))).IsEqualTo("5,2,1");
        await Assert.That(filtered[0].Enabled).IsFalse();
        await Assert.That(filtered[0].Pinned).IsTrue();
    }

    private static Assignment Clone(Assignment assignment) => new Assignment(assignment.RoleId, assignment.Enabled, assignment.Pinned);
