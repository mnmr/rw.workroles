...
public class VanillaProjectionMetadataTests
{
    ...
    {
        var definitions = new VanillaProjectionDefinitionMetadata([Source("Doctor", skilled: true), Source("Hauling"), Source("Research", skilled: true, research: true)]);
        var metadata = definitions.WithBasics([]);
    ...
    public async Task CategoriesPreserveBasicsSkilledResearchAndGruntSemantics()
    {
        ...
        var metadata = definitions.WithBasics(["BasicsUnskilled"]);
    ...
    {
        var definitions = new VanillaProjectionDefinitionMetadata([Source("A"), Source("A", skilled: true, research: true), Source("B", skilled: true)]);
        var metadata = definitions.WithBasics([]);
    ...
    {
        var definitions = new VanillaProjectionDefinitionMetadata([Source("A"), Source("B")]);
        var metadata = definitions.WithBasics(["Missing", "A", "Missing", "A"]);
    ...
    {
        var definitions = new VanillaProjectionDefinitionMetadata([Source("A"), Source("HiddenSkilled", skilled: true), Source("B")], ["B", "A", "B"]);
        var metadata = definitions.WithBasics([]);
    ...
    {
        List<VanillaProjectionWorkTypeSource> sources = [Source("A")];
        List<string> priorityOrder = ["A"];
        List<string> basics = ["A"];
        ...
        await Assert.That(() => ((IDictionary<string, int>)metadata.Columns).Add("B", 1)).Throws<NotSupportedException>();
        await Assert.That(() => ((IList<string>)metadata.WorkTypes).Add("B")).Throws<NotSupportedException>();
        await Assert.That(() => ((IList<string>)metadata.Basics).Clear()).Throws<NotSupportedException>();
    }

    private static VanillaProjectionWorkTypeSource Source(string name, bool skilled = false, bool research = false) => new VanillaProjectionWorkTypeSource(name, skilled, research);
}
