...
public class SwatchPickPlannerTests
{
    ...
    private static List<Rgba> Custom() => [C0, C1];

    /// Custom-colored roles only: 1 follows slot 0, 4 follows slot 1,
    /// 3 wears a standard swatch (picks must leave it untouched).
    private static List<(int id, Rgba color)> Roles() => [(1, C0), (3, S2), (4, C1)];

    [Test]
    public async Task ReacceptingTheSlotsOwnColorChangesNothing()
    {
        var plan = SwatchPickPlanner.Plan(C0, slot: 0, Standard, Custom(), applyToEditedRole: false, editedRoleId: 1, Roles());
    ...
    {
        var plan = SwatchPickPlanner.Plan(S2, slot: 0, Standard, Custom(), applyToEditedRole: false, editedRoleId: 1, Roles());
        ...
        await Assert.That(plan.RecolorRoleIds).IsEquivalentTo([1]);
        await Assert.That(plan.RecolorEditedRole).IsFalse();
    }
    ...
    public async Task RedefiningToANewCustomColorMovesFollowersWithTheSlot()
    {
        ...
        var plan = SwatchPickPlanner.Plan(picked, slot: 0, Standard, Custom(), applyToEditedRole: false, editedRoleId: 1, Roles());
        await Assert.That(plan.ClearSlot).IsFalse();
        await Assert.That(plan.SetSlot).IsTrue();
    ...
    {
        List<Rgba> custom = [C0, C1, Undefined];
        var nearS1 = new Rgba(0.201f, 0.4f, 0.6f, 1f);
        var plan = SwatchPickPlanner.Plan(nearS1, slot: 2, Standard, custom, applyToEditedRole: true, editedRoleId: 3, Roles());
        await Assert.That(plan.ClearSlot).IsFalse();
        await Assert.That(plan.SetSlot).IsFalse();
    ...
    {
        var plan = SwatchPickPlanner.Plan(C0, slot: 1, Standard, Custom(), applyToEditedRole: false, editedRoleId: 4, Roles());
        await Assert.That(plan.ClearSlot).IsTrue();
        await Assert.That(plan.SetSlot).IsFalse();
        await Assert.That(plan.Applied.Matches(C0)).IsTrue();
        await Assert.That(plan.RecolorRoleIds).IsEquivalentTo([4]);
    }
    ...
    public async Task AppliedPlansNeverStrandARoleColorOutsideThePalette(float r, float g, float b, int slot, bool applyToEditedRole, int editedRoleId)
    {
        var picked = new Rgba(r, g, b, 1f);
        var custom = Custom();
        var roles = Roles();
        var plan = SwatchPickPlanner.Plan(picked, slot, Standard, custom, applyToEditedRole, editedRoleId, roles);

        if (plan.ClearSlot)
            custom[slot] = Undefined;
        if (plan.SetSlot)
        {
            while (custom.Count <= slot)
                custom.Add(Undefined);
            custom[slot] = plan.Applied;
        }
        for (int i = 0; i < roles.Count; i++)
            if (plan.RecolorRoleIds.Contains(roles[i].id) || (plan.RecolorEditedRole && roles[i].id == editedRoleId))
                roles[i] = (roles[i].id, plan.Applied);

        foreach (var (_, color) in roles)
        {
            bool inPalette = Standard.Any(s => s.Matches(color)) || custom.Any(c => c.Defined && c.Matches(color));
            await Assert.That(inPalette).IsTrue();
        }
    }

    [Test]
    public async Task MatchesAcceptsColorsWithIdenticalByteQuantization()
    {
        var baseColor = new Rgba(0.2f, 0.4f, 0.6f, 1f);

        await Assert.That(baseColor.Matches(new Rgba(0.201f, 0.401f, 0.601f, 1f))).IsTrue();
    }

    [Test]
    public async Task MatchesRejectsColorsOutsideTheSumTolerance()
    {
        var baseColor = new Rgba(0.2f, 0.4f, 0.6f, 1f);

        await Assert.That(baseColor.Matches(new Rgba(0.206f, 0.4f, 0.6f, 1f))).IsFalse();
    }
}
