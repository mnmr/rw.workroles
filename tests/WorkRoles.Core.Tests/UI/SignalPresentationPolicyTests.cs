...
public class SignalPresentationPolicyTests
{
    ...
    public async Task ForSkillKeepsSkillAndGlobalSignalsSeparate()
    {
        ...
        var global = MakeSignal("FastLearner", "Ludeon.RimWorld", null, null, kind: SignalSourceKind.Trait);
        var melee = MakeSignal("Brawler", "Ludeon.RimWorld", "Melee", null, kind: SignalSourceKind.Trait);

        Signal[] signals = [melee, global, shooting];
        var view = SignalPresentationPolicy.ForSkill(new SignalSnapshot(signals), "Shooting");

        await Assert.That(view.SkillSignals).IsEquivalentTo([shooting]);
        await Assert.That(view.GlobalSignals).IsEquivalentTo([global]);
        await Assert.That(view.IconCandidates).IsEquivalentTo([shooting]);
    ...
    {
        var officialMissing = MakeSignal("Brawler", "Ludeon.RimWorld", "Shooting", null, kind: SignalSourceKind.Trait);
        var dlcMissing = MakeSignal("Inhumanized", "ludeon.rimworld.anomaly", "Shooting", "", kind: SignalSourceKind.Hediff);
        var modMissing = MakeSignal("VSE_Test", "vanillaexpanded.skills", "Shooting", null, kind: SignalSourceKind.Expertise);
        var modIcon = MakeSignal("AS_Test", "sarg.alphaskills", "Shooting", "Passions/AS_Test");

        Signal[] signals = [modMissing, officialMissing, modIcon, dlcMissing];
        var view = SignalPresentationPolicy.ForSkill(new SignalSnapshot(signals), "Shooting");

        await Assert.That(view.IconCandidates).IsEquivalentTo([modIcon]);
        await Assert.That(view.OfficialMissingIcons).IsEquivalentTo([officialMissing, dlcMissing]);
    ...
    public async Task PassionTierUsesTheStrongestAuthoredPassionTier()
    {
        ...
        var trait = MakeSignal("Brawler", "Ludeon.RimWorld", "Shooting", null, "Major", SignalSourceKind.Trait);

        Signal[] forwardSignals = [minor, trait, major];
        Signal[] reverseSignals = [major, trait, minor];
        var forward = SignalPresentationPolicy.ForSkill(new SignalSnapshot(forwardSignals), "Shooting");
        var reverse = SignalPresentationPolicy.ForSkill(new SignalSnapshot(reverseSignals), "Shooting");
    ...
    public async Task SpilloversRemainTooltipFactsButNeverBecomeDecoratorsOrPassionColour()
    {
        var spillover = new Signal(
            ...
            "Cooking",
            [new SignalEffect(SignalEffectKind.LearningRate, SignalOperation.Multiply, 0.25f, SignalValueUnit.Factor, "Cooking")],
            new SignalUi("critical", null, "Passions/PassionCritical", "Major", "Major", "Vanilla Skills Expanded"),
            originSkillDefName: "Crafting",
            relation: SignalRelation.Spillover
        );

        Signal[] signals = [spillover];
        SkillSignalView view = SignalPresentationPolicy.ForSkill(new SignalSnapshot(signals), "Cooking");

        await Assert.That(view.SkillSignals).IsEquivalentTo([spillover]);
        await Assert.That(view.IconCandidates.Count).IsEqualTo(0);
    ...
    public async Task ActivePassiveSplitPreservesOrderWithinEachSource()
    {
        ...
        Signal[] signals = [activeSkill1, passiveSkill, activeSkill2, activeGlobal, passiveGlobal];
        var view = SignalPresentationPolicy.ForSkill(new SignalSnapshot(signals), "Shooting");
        ...
        await Assert.That(string.Join(",", view.ActiveSignals.Select(x => x.Source.DefName))).IsEqualTo("A1,A2,AG1");
        await Assert.That(string.Join(",", view.PassiveSignals.Select(x => x.Source.DefName))).IsEqualTo("P1,PG1");
    }
    ...
    public async Task SpilloversIncludedInActivePassiveLists()
    {
        ...
        Signal[] signals = [spillover, activeGlobal];
        var view = SignalPresentationPolicy.ForSkill(new SignalSnapshot(signals), "Cooking");

        await Assert.That(string.Join(",", view.ActiveSignals.Select(x => x.Source.DefName))).IsEqualTo("VSE_Critical,AG1");
        await Assert.That(view.PassiveSignals.Count).IsEqualTo(0);
    }
    ...
    ) => new Signal(type, new SignalSource(kind, defName, packageId), skill, [], new SignalUi(defName, null, icon, tier, tier, packageId));
}
