...
public class RoleDefVanillaTests
{
    ...
    private static readonly Dictionary<string, string> GiverType = VanillaWorkOrder.GiversInOrder.SelectMany(kv => kv.Value.Select(g => (g, kv.Key))).ToDictionary(x => x.g, x => x.Key);
    ...
    public async Task EveryColorRefResolvesInThePalette()
    {
        ...
        foreach (var def in XElement.Load(Path.Combine(defsDir, "Roles.xml")).Elements("WorkRoles.RoleDef"))
        {
            ...
            if (colorRef == null)
                continue;
            await Assert.That(palette.Contains(colorRef)).IsTrue().Because($"{def.Element("defName")?.Value}: unknown colorRef '{colorRef}'");
        }
    }
    ...
    public async Task EveryGroupReferenceResolves()
    {
        ...
        foreach (var def in root.Elements("WorkRoles.RoleDef"))
        {
            ...
            if (group != null)
                await Assert.That(groups.Contains(group)).IsTrue().Because($"{name}: unknown group '{group}'");
        }
    }
    ...
    public async Task EveryEntryExistsInVanilla()
    {
        ...
        {
            var known = entry.Kind == JobEntryKind.WorkType ? VanillaWorkOrder.NaturalPriority.ContainsKey(entry.DefName) : GiverType.ContainsKey(entry.DefName);
            await Assert.That(known).IsTrue().Because($"{role.DefName}: '{entry.DefName}' is not a vanilla {entry.Kind}");
        }
    }
    ...
    public async Task VanillaWorkOrderAndGiverBaselineAgree()
    {
        foreach (var pair in GiverType)
            await Assert
                .That(VanillaGiverBaseline.GiverWorkType.TryGetValue(pair.Key, out var type) && type == pair.Value)
                .IsTrue()
                .Because($"{pair.Key}: VanillaWorkOrder says {pair.Value}, baseline says " + (VanillaGiverBaseline.GiverWorkType.TryGetValue(pair.Key, out var t) ? t : "<missing>"));
        foreach (var pair in VanillaGiverBaseline.GiverWorkType)
            await Assert.That(GiverType.ContainsKey(pair.Key)).IsTrue().Because($"{pair.Key} is in the baseline but not in VanillaWorkOrder");
    }
    ...
    {
        var coveredTypes = Roles.SelectMany(r => r.Entries).Where(e => e.Kind == JobEntryKind.WorkType).Select(e => e.DefName).ToHashSet();
        foreach (var type in VanillaWorkOrder.NaturalPriority.Keys)
            await Assert.That(coveredTypes).Contains(type).Because($"no role covers work type {type}");

        var coveredGivers = Roles.SelectMany(r => r.Entries).SelectMany(e => e.Kind == JobEntryKind.WorkType ? VanillaWorkOrder.GiversInOrder[e.DefName] : [e.DefName]).ToHashSet();
        foreach (var giver in GiverType.Keys)
            await Assert.That(coveredGivers).Contains(giver).Because($"no role covers giver {giver}");
    }
    ...
    public async Task ParentRolesCoverTheirChildren(string parent, string child)
    {
        ...
        await Assert.That(CoverageMath.Covers(parentCoverage, childCoverage)).IsTrue().Because($"{child}'s coverage is not strictly inside {parent}'s");
    }
}
