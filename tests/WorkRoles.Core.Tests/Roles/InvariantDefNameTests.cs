...
public class InvariantDefNameTests
{
    ...
    public async Task HumanizeUsesOnlyInvariantDefName(string defName, string prefix, string expected)
    {
        await Assert.That(InvariantDefName.Humanize(defName, prefix)).IsEqualTo(expected);
    }
}
