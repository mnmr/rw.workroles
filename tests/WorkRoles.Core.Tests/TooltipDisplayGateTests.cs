...
public class TooltipDisplayGateTests
{
    ...
    public async Task ContinuousHoverOpensOnceAndThenRemainsVisible()
    {
        ...
        await Assert.That(gate.Observe("role:builder", 10, 1f, 0.45f))
            .IsEqualTo(TooltipDisplayState.Pending);
        await Assert.That(gate.Observe("role:builder", 11, 1.44f, 0.45f))
            .IsEqualTo(TooltipDisplayState.Pending);
        await Assert.That(gate.Observe("role:builder", 12, 1.45f, 0.45f))
            .IsEqualTo(TooltipDisplayState.Opened);
        await Assert.That(gate.Observe("role:builder", 13, 1.46f, 0.45f))
    ...
    public async Task GapKeyChangeAndResetStartANewDelay()
    {
        ...
        await Assert.That(gate.Observe("role:builder", 13, 2f, 0.45f))
            .IsEqualTo(TooltipDisplayState.Pending);
        await Assert.That(gate.Observe("role:crafter", 14, 3f, 0.45f))
            .IsEqualTo(TooltipDisplayState.Pending);

        gate.Reset();
        await Assert.That(gate.Observe("role:crafter", 15, 4f, 0.45f))
            .IsEqualTo(TooltipDisplayState.Pending);
    }
    ...
    public async Task SuppressionPreventsPresentationWhileHoverContinues()
    {
        ...
        await Assert.That(gate.Observe("role:builder", 12, 1.46f, 0.45f))
            .IsEqualTo(TooltipDisplayState.Suppressed);
    }
    ...
    public async Task ReleasingSuppressionRestartsTheHoverDelay()
    {
        ...
        await Assert.That(gate.Observe("role:builder", 12, 2f, 0.45f))
            .IsEqualTo(TooltipDisplayState.Pending);
        await Assert.That(gate.Observe("role:builder", 13, 2.45f, 0.45f))
            .IsEqualTo(TooltipDisplayState.Opened);
    }
