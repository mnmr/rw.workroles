...
public class MoreThanCapableSignalTests
{
    ...
    {
        Signal signal = MoreThanCapableSignal.Create("Cooking", "hated cooking", "Causes a severe mood penalty.");
        ...
        await Assert.That(signal.Source.PackageId).IsEqualTo("void.MoreThanCapable");
        await Assert.That(signal.WorkTypeDefName).IsEqualTo("Cooking");
        await Assert.That(signal.SkillDefName == null).IsTrue();
        await Assert.That(signal.Effects.Single().Kind).IsEqualTo(SignalEffectKind.WorkPreference);
    ...
    {
        await Assert.That(() => MoreThanCapableSignal.Create(" ", "hated work", "Severe mood penalty.")).Throws<ArgumentException>();
    }
}
