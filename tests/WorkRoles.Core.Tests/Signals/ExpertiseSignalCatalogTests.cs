...
public class ExpertiseSignalCatalogTests
{
    ...
    [Test]
    public void VanillaSkillsExpandedMembershipSkillsAndStatOffsetsAreExact()
    {
        AssertRows("vanillaexpanded.skills", VseRows);
    }

    [Test]
    public void AlphaSkillsMembershipSkillsAndStatOffsetsAreExact()
    {
        AssertRows("sarg.alphaskills", AlphaRows);
    }

    [Test]
    public async Task EveryExpertiseDefinitionIsActivePersistentAndScaledByExpertiseLevel()
    {
        await Assert.That(ExpertiseSignalDefinitions.All.All(x => x.Type == SignalType.Active && !x.IsTransient && x.Effects.All(e => e.ScaleKind == SignalScaleKind.ExpertiseLevel))).IsTrue();
    }

    [Test]
    [Arguments("AS_Blasting", "vanillaexpanded.skills")]
    [Arguments("Hacker", "Ludeon.RimWorld.Ideology")]
    [Arguments("AS_Psycasting", "Ludeon.RimWorld.Royalty")]
    [Arguments("AS_Piloting", "Ludeon.RimWorld.Odyssey")]
    public async Task ExpertiseCarriesItsRequiredPackage(string defName, string requiredPackageId)
    {
        await Assert.That(One(defName).Source.RequiredPackageIds).Contains(requiredPackageId);
    }

    [Test]
    public async Task WardenSuppressionEffectRequiresIdeology()
    {
        var suppression = One("Warden").Effects.Single(x => x.TargetDefName == "SuppressionPower");

        await Assert.That(suppression.Conditions.Any(x => x.Key == "package:Ludeon.RimWorld.Ideology")).IsTrue();
    }

    [Test]
    [Arguments("Precision", "ShootingAccuracyPawn", SignalEffectKind.Accuracy)]
    [Arguments("Foreman", "ConstructionSpeed", SignalEffectKind.WorkSpeed)]
    [Arguments("QualityExpert", "VSE_ArtQuality", SignalEffectKind.Quality)]
    [Arguments("AS_CraftingYield", "AS_CraftingYield", SignalEffectKind.Yield)]
    [Arguments("AS_Cleaving", "MeleeDamageFactor", SignalEffectKind.Damage)]
    [Arguments("AS_Evading", "MeleeDodgeChance", SignalEffectKind.Dodge)]
    public async Task StatReceivesItsSpecificEffectKind(string defName, string statDefName, SignalEffectKind expectedKind)
    {
        await Assert.That(Effect(defName, statDefName).Kind).IsEqualTo(expectedKind);
    }
    ...
    public async Task DescriptionOnlyOverrideKeepsTheExpertiseStarIcon()
    {
        ...
        Signal signal = SignalFactory.Instantiate(definition, currentScale: 2f, ui: new SignalUiOverride(description: "Runtime full description"));
    ...
    {
        var actual = ExpertiseSignalDefinitions.All.Where(x => x.Source.PackageId == packageId).Select(Row).OrderBy(x => x, StringComparer.Ordinal).ToArray();
        var expected = expectedRows.Select(NormalizeRow).OrderBy(x => x, StringComparer.Ordinal).ToArray();
    ...
    {
        var effects = definition.Effects.OrderBy(x => x.TargetDefName, StringComparer.Ordinal).Select(x => x.TargetDefName + "=" + x.Magnitude.Value.ToString("0.###", CultureInfo.InvariantCulture));
    ...
    private static SignalDefinition One(string defName) => ExpertiseSignalDefinitions.All.Single(x => x.Source.DefName == defName);

    private static SignalEffect Effect(string defName, string statDefName) => One(defName).Effects.Single(x => x.TargetDefName == statDefName);
}
