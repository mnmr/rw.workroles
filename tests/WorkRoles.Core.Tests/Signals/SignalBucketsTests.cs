...
public class SignalBucketsTests
{
    ...
    public async Task TiersOrderAsSpecified()
    {
        ...
        string ladder = string.Join(",", Enum.GetValues(typeof(SignalBucket)).Cast<SignalBucket>().OrderBy(x => (int)x).Select(x => $"{(int)x}:{x}"));
        await Assert.That(ladder).IsEqualTo("0:Awful,1:Poor,2:Neutral,3:Strong,4:Great,5:Exceptional");
    }
}
