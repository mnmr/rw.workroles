...
/// mode/scale derivation instead of asserting them into existence.
internal sealed class RecsProjection
{
    ...
    internal RecsProjection WorkType(
        string name, string skill, int naturalPriority, params string[] givers)
    {
        if (givers.Length == 0) givers = new[] { name + "_g" };
        jobs.WithWorkType(name, givers);
        JobProfileSkillSource[] skills = skill != null
            ? new[] { new JobProfileSkillSource(10, skill) }
            : Array.Empty<JobProfileSkillSource>();
        string[] curated = skill != null
            ? new[] { skill }
            : Array.Empty<string>();
        profiles.AddWorkType(nextWorkTypeId, name, skills, givers);
        foreach (string giver in givers)
            profiles.AddGiver(
                giver, nextWorkTypeId, skills,
                hasCuratedXp: true, curatedXpSkillDefNames: curated);
        naturalPriorities[name] = naturalPriority;
        nextWorkTypeId++;
        return this;
    }

    internal RecommendationRoleSource RoleByWorkType(
        int id, ScaleMode mode, HolderScale scale, params string[] workTypes)
    {
        var source = new RecommendationRoleSource
        {
            Id = id, Mode = mode, Scale = scale,
        };
    ...
    internal RecommendationRoleSource RoleByGiver(
        int id, ScaleMode mode, HolderScale scale, params string[] givers)
    {
        var source = new RecommendationRoleSource
        {
            Id = id, Mode = mode, Scale = scale,
    ...
    internal RecommendationPlan Plan(params PawnView[] pawns)
    {
        RecommendationCatalogProjection projection =
            RecommendationCatalogBuilder.Build(
                sources, paths, jobs, naturalPriorities, profiles.Build());
        ColonyView colony = projection.CreateColony(
            sources.Select(s => s.Id), pawns);
        return RecommendationPlan.Build(colony);
    }

    internal static HolderScale Scale(
        int requiredTotal = 0, int trainingWaivers = 0,
        int max = RoleHolderRange.Uncapped)
    {
        var scale = new HolderScale();
        Array.Fill(scale.RequiredTotals, requiredTotal);
        Array.Fill(scale.TrainingWaivers, trainingWaivers);
        Array.Fill(scale.Max, max);
        scale.Normalize();
        return scale;
    }
