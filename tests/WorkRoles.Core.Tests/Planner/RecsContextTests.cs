...
public class RecsContextTests
{
    ...
    public async Task BestSignalUsesOrdinalRoleSkillTieBreakRegardlessOfInputOrder(string orderedSkills)
    {
        ...
        try
        {
            ...
            string result = RoleSkill(orderedSkills.Split(','));
    ...
    public async Task BasePositionsUseTemplateSlotsAndNaturalFallback()
    {
        ...
        RecsTestBed.SetNaturalPriority(first, 100);
        RoleView second = RecsTestBed.Role(2, "Crafting");
        RecsTestBed.SetNaturalPriority(second, 50);
        RoleView unlisted = RecsTestBed.Role(3, "Doctor");
        RecsTestBed.SetNaturalPriority(unlisted, 60);
    ...
    private static string RoleSkill(IReadOnlyList<string> sourceOrder)
    {
        RoleView role = RecsTestBed.Unskilled(1, "Hauling");
        var givers = new List<RoleWorkGiverSpec>();
        foreach (string candidate in sourceOrder)
            givers.Add(RecsTestBed.Giver($"{candidate}Work", used: [candidate]));
        RecsTestBed.SetSpec(role, RecsTestBed.Capability("Hauling", 0, [.. givers]));
        PawnView pawn = RecsTestBed.Pawn();
        ...
        var context = new EngineContext(RecsTestBed.Colony([role], pawn));
