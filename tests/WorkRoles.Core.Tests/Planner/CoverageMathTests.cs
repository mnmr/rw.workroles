...
{
    private static readonly FakeCatalog Catalog = new FakeCatalog().WithWorkType("Cooking", "Cook", "Butcher", "Brew").WithWorkType("Hauling", "HaulGeneral");
    ...
    {
        var coverage = CoverageMath.CoverageOf([Type("Cooking"), Giver("HaulGeneral"), Giver("ModdedGone"), Type("ModdedType")], Catalog);
        await Assert.That(coverage).IsEquivalentTo(["Cook", "Butcher", "Brew", "HaulGeneral"]);
    }
    ...
    public async Task OrderedCoverageFollowsEntryOrderExpandsInCatalogOrderAndDedupes()
    {
        ...
        var ordered = CoverageMath.OrderedCoverageOf([Giver("Butcher"), Type("ModdedType"), Type("Cooking"), Giver("HaulGeneral")], Catalog);
    ...
    {
        var cookType = CoverageMath.CoverageOf([Type("Cooking")], Catalog);
        var butcher = CoverageMath.CoverageOf([Giver("Butcher")], Catalog);
    ...
    {
        var asType = CoverageMath.CoverageOf([Type("Cooking")], Catalog);
        var asGivers = CoverageMath.CoverageOf([Giver("Cook"), Giver("Butcher"), Giver("Brew")], Catalog);
    ...
    {
        var empty = CoverageMath.CoverageOf([], Catalog);
        var cook = CoverageMath.CoverageOf([Type("Cooking")], Catalog);
    ...
    [Test]
    public async Task ImmediateCoverageShadowsDescendantsButRetainsIncomparableAndEqualCoverage()
    {
        var cooking = CoverageMath.CoverageOf([Type("Cooking")], Catalog);
        var butcherBrew = CoverageMath.CoverageOf([Giver("Butcher"), Giver("Brew")], Catalog);
        var butcher = CoverageMath.CoverageOf([Giver("Butcher")], Catalog);
        var haul = CoverageMath.CoverageOf([Giver("HaulGeneral")], Catalog);
        var cookingAgain = CoverageMath.CoverageOf([Giver("Cook"), Giver("Butcher"), Giver("Brew")], Catalog);

        var immediate = CoverageMath.ImmediatelyCoveredIndexes([cooking, butcherBrew, butcher, haul, cookingAgain]);

        await Assert.That(immediate).IsEquivalentTo([0, 3, 4]);
    }

    [Test]
    public async Task ImmediateCoverageSurfacesTheLargestAvailableSubset()
    {
        var butcherBrew = CoverageMath.CoverageOf([Giver("Butcher"), Giver("Brew")], Catalog);
        var butcher = CoverageMath.CoverageOf([Giver("Butcher")], Catalog);
        var haul = CoverageMath.CoverageOf([Giver("HaulGeneral")], Catalog);

        var reduced = CoverageMath.ImmediatelyCoveredIndexes([butcherBrew, butcher, haul]);

        await Assert.That(reduced).IsEquivalentTo([0, 2]);
    }

    [Test]
    public async Task ImmediateCoverageOfNoCandidatesIsEmpty()
    {
        await Assert.That(CoverageMath.ImmediatelyCoveredIndexes([])).IsEmpty();
    }

    [Test]
    public async Task FirstCoveredIndexFindsTheEarliestGiverOrMaxValue()
    {
        var ordered = CoverageMath.OrderedCoverageOf([Type("Cooking"), Giver("HaulGeneral")], Catalog);
        var brewHaul = CoverageMath.CoverageOf([Giver("Brew"), Giver("HaulGeneral")], Catalog);
        var haul = CoverageMath.CoverageOf([Giver("HaulGeneral")], Catalog);
        var none = CoverageMath.CoverageOf([Giver("ModdedGone")], Catalog);
