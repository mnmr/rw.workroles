...
public class RecommendationRoleProjectionTests
{
    ...
    {
        List<RecommendationWorkTypeEvidence> workTypes = [new("Crafting", 7), new("Hunting", 3), new("Crafting", 99)];
        List<string> literals = ["MissingModType", "Hunting", "Hunting"];

        var projection = new RecommendationRoleProjection(workTypes, literals, []);
        workTypes.Clear();
        literals.Clear();

        await Assert.That(string.Join(",", projection.WorkTypes)).IsEqualTo("Crafting,Hunting");
        ...
        await Assert.That(() => ((IList<string>)projection.WorkTypes).Add("Mining")).Throws<NotSupportedException>();
        await Assert.That(() => ((IDictionary<string, int>)projection.NaturalPriorities).Add("Mining", 2)).Throws<NotSupportedException>();
    }

    [Test]
    public async Task ProjectionOwnsAggregatedSkillEvidenceAndReturnsFreshMutableViews()
    {
        List<RoleSkillEvidence> evidence =
        [
        ...
        var projection = new RecommendationRoleProjection([], [], evidence);
        evidence.Clear();
        ...
        await Assert.That(projection.SkillEvidence.Single(skill => skill.Primary).SkillDefName).IsEqualTo("Crafting");
        ...
        await Assert.That(second.Single(skill => skill.Primary).SkillDefName).IsEqualTo("Crafting");
    }
}
