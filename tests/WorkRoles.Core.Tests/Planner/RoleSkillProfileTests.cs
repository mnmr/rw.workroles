...
public class RoleSkillProfileTests
{
    ...
    {
        List<RoleSkillView> profile = RoleSkillProfile.Build([
        ...
        await Assert.That(profile.Select(s => s.SkillDefName)).IsEquivalentTo(["Crafting", "Intellectual"]);
    ...
    [Test]
    public async Task PrimaryTieBreakPrefersMoreTrainingEvidence()
    {
        List<RoleSkillView> profile = RoleSkillProfile.Build([
    ...
    [Test]
    public async Task PrimaryTieBreakUsesOrdinalSkillNameWhenEvidenceIsEqual()
    {
        List<RoleSkillView> profile = RoleSkillProfile.Build([
        ...
        await Assert.That(profile.Single(s => s.Primary).SkillDefName).IsEqualTo("Cooking");
    }
    ...
    public async Task MergeKeepsEverySpecialistSkillTheShareFilterWouldCollapse()
    {
        ...
        RoleSkillEvidence[] doctorEvidence = [new RoleSkillEvidence("Medicine", usedJobs: 6, trainedJobs: 6, requiredContent: 0, weightedJobs: 24)];
        RoleSkillEvidence[] growerEvidence = [new RoleSkillEvidence("Plants", usedJobs: 4, trainedJobs: 4, requiredContent: 0, weightedJobs: 16)];

        // Filtered over the union, Plants falls under the half-share bar.
        List<RoleSkillView> collapsed = RoleSkillProfile.Build(doctorEvidence.Concat(growerEvidence), roleWeight: 24 + 16);
        await Assert.That(collapsed.Select(s => s.SkillDefName)).IsEquivalentTo(["Medicine"]);

        List<RoleSkillView> merged = RoleSkillProfile.Merge([RoleSkillProfile.Build(doctorEvidence, roleWeight: 24), RoleSkillProfile.Build(growerEvidence, roleWeight: 16)]);
        await Assert.That(string.Join(",", merged.Select(s => s.SkillDefName))).IsEqualTo("Medicine,Plants");
        await Assert.That(merged.Single(s => s.Primary).SkillDefName).IsEqualTo("Medicine");
    }
}
