...
public class IdentitySelectionPreserverTests
{
    ...
    public async Task RestoreCarriesSelectionOnlyAcrossTheSameReferenceIdentity()
    {
        ...
        Item[] prior = [new Item(firstPawn, included: false), new Item(equalButDifferentPawn, included: false)];
        Item[] refreshed = [new Item(firstPawn, included: true), new Item(new object(), included: true)];
    ...
    {
        var pawn = new object();
        Item[] entries = [new(pawn, included: false)];
        Dictionary<object, bool> captured = IdentitySelectionPreserver.Capture(entries, item => item.Identity, item => item.Included, ReferenceIdentityComparer<object>.Instance);

        entries[0].Included = true;
        int included = IdentitySelectionPreserver.Restore(captured, entries, item => item.Identity, item => item.Included, (item, value) => item.Included = value);
