...
public class IdentityKeySweepPlannerTests
{
    ...
    public async Task ReferenceIdentityDictionaryKeepsValueEqualKeysSeparateAndAliasesStable()
    {
        ...
        var alias = first;
        var mapped = new Dictionary<IdentityKey, int>(ReferenceIdentityComparer<IdentityKey>.Instance) { [first] = 10, [equalButDifferentReference] = 20 };
    ...
    public async Task StaleKeysUseReferenceIdentityAndIgnoreNullDuplicateLiveRefs()
    {
        ...
        IReadOnlyList<IdentityKey> planned = IdentityKeySweepPlanner.StaleKeys(stored, liveKeys);
    ...
    public async Task ApplyingThePlanMakesTheNextSweepIdempotent()
    {
        var live = new IdentityKey(1);
        var stale = new IdentityKey(2);
        List<IdentityKey> stored = [live, stale];

        IReadOnlyList<IdentityKey> first = IdentityKeySweepPlanner.StaleKeys(stored, [live]);
        stored.RemoveAll(candidate => first.Any(planned => ReferenceEquals(candidate, planned)));
        IReadOnlyList<IdentityKey> second = IdentityKeySweepPlanner.StaleKeys(stored, [live, live]);
    ...
    public async Task HashSetOverloadDoesNotTrustValueEqualityComparer()
    {
        var live = new IdentityKey(1);
        var equalButDifferentReference = new IdentityKey(1);
        HashSet<IdentityKey> valueEqualitySet = [live];

        IReadOnlyList<IdentityKey> planned = IdentityKeySweepPlanner.StaleKeys([equalButDifferentReference], valueEqualitySet);
    ...
    private sealed class IdentityKey
    {
        ...
        public override bool Equals(object obj) => obj is IdentityKey other && other.value == value;
